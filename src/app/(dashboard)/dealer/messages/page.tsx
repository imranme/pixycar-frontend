...
function DealerMessagesContent() {
  ...
  const {
    ...
    sendMessage,
    sendTyping,
  } = useChatWebSocket(activeConvo?.id || activeConvoId);
  ...
  const displayedMessages: ChatMessage[] = useMemo(() => {
    ...
    const serverMsgs: ChatMessage[] = msgsList.map((m) => {
      ...
      return {
        id: m.id,
        sender: (isMe ? "dealer" : "seller") as "dealer" | "seller",
        text: m.text || "",
        time: m.created_at
  ...
  if (!activeConvo) {
    return (
      <div className="flex min-h-[400px] flex-col items-center justify-center gap-3">
        <p className="font-navbar text-base text-[#5E5E5E]">No conversations yet.</p>
      </div>
    );
  }
      ...
      <div className="flex h-[calc(100dvh-9.5rem)] min-h-[480px] overflow-hidden rounded-2xl border border-[#E5E7EB] bg-white shadow-xs">
        ...
        >
          <ChatWindow
            ...
            currentRole="dealer"
            dealerPartyInitial="D"
            sellerPartyInitial={activeConvo.sellerInitial ?? "S"}
