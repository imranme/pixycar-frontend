...
export default function DealerBiddingPage({ params }: PageProps) {
  ...
  if (apiListing && (apiListing as any).id) {
    ...
    const isTimeOver = (l.time_remaining_seconds !== undefined && l.time_remaining_seconds <= 0) || l.status === "TIME_OVER" || l.status === "EXPIRED" || l.status === "SOLD";
  ...
  if (!listing && !isLoading) {
    return (
      <div className="mx-auto flex min-h-[50vh] max-w-md flex-col items-center justify-center gap-4 px-4 text-center">
        ...
        <p className="font-navbar text-sm text-[#5E5E5E]">The requested vehicle listing could not be found or has expired.</p>
        <Link
