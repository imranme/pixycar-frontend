...
import { useUpdateDealerProfileMutation } from "@/store/features/auth/authApi";

export default function DealerEditProfilePage() {
  const user = useAppSelector(selectCurrentUser);
  const [toast, setToast] = useState<string | null>(null);
  const [updateProfile, { isLoading }] = useUpdateDealerProfileMutation();
  ...
  const {
    ...
    defaultValues: {
      fullName: user?.business_name || user?.full_name || user?.name || "",
      phone: "",
      address: "",
    },
    mode: "onTouched",
